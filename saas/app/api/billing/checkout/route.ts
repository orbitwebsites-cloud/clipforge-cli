...
export async function POST(request: Request) {
  try {
    const { plan, billingCycle } = z.object({ plan: z.enum(['creator', 'studio']), billingCycle: z.enum(['monthly', 'annual']).default('monthly') }).parse(await request.json());
    const tenantId = await tenantIdFromSession();
    if (plan === 'creator') {
      const whopUrl = billingCycle === 'annual' ? process.env.WHOP_CREATOR_ANNUAL_CHECKOUT_URL : process.env.WHOP_CREATOR_CHECKOUT_URL;
    ...
    const price = plan === 'studio' ? process.env.STRIPE_PRICE_STUDIO : billingCycle === 'annual' ? process.env.STRIPE_PRICE_CREATOR_ANNUAL : process.env.STRIPE_PRICE_CREATOR;
