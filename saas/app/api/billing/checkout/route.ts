...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.STRIPE_SECRET_KEY) return Response.json({ url: '/success?type=checkout&mode=demo' });
    ...
    const session = await stripe.checkout.sessions.create({ mode: 'subscription', customer: tenant.stripeCustomerId || undefined, customer_email: tenant.stripeCustomerId ? undefined : tenant.email, line_items: [{ price, quantity: 1 }], success_url: `${appUrl()}/success?type=checkout`, cancel_url: `${appUrl()}/cancelled?from=checkout`, allow_promotion_codes: true, subscription_data: { metadata: { tenantId, plan, billingCycle } }, metadata: { tenantId, plan, billingCycle } });
