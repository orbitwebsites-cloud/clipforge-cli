        ...
        {!data.sourceChannels.length ? <SourceOnboarding destination={destination.title} sourceUrl={sourceUrl} setSourceUrl={setSourceUrl} addSource={addSource} saving={saving} notice={notice} /> : <>
          ...
          <div className="dashboard-columns">
            ...
            <section className="panel setup-panel" id="channels"><div className="panel-header"><div><p className="overline">Input channels</p><h2>Clip sources</h2></div><span className="step-count">{data.sourceChannels.length} active</span></div>
              ...
              <form className="channel-form" onSubmit={addSource}><label htmlFor="channel"><Link2 /> Add a YouTube or Twitch source</label><div><input id="channel" value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="youtube.com/@creator or twitch.tv/creator" required /><button disabled={saving || data.sourceChannels.length >= data.tenant.sourceChannelLimit}>{saving ? <LoaderCircle className="spin" /> : <Plus />}</button></div><small>YouTube uploads and stream replays, plus Twitch VODs after streams end, are monitored automatically.</small></form>{notice && <p className="form-notice">{notice}</p>}
            </section>
          </div>
        </>}
