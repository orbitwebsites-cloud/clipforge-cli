...
import type { ChannelAnalytics, CreatorPreferences, DashboardData, Job, JobStatus, PastVideo } from '@/lib/types';
  ...
  return <main className="app-shell">
    ...
    <section className="app-main">
      ...
      {activeTab === 'profile' ? <ProfilePanel data={data} /> : activeTab === 'settings' ? <SettingsPanel initial={data.preferences} onSaved={(dashboard) => setData(dashboard)} /> : activeTab === 'billing' ? <BillingPanel data={data} saving={saving} startCheckout={startCheckout} /> : !destination ? <EmptyOnboarding name={data.tenant.name} /> : <>
        ...
        {activeTab === 'sources' && <SourcesPanel data={data} onDashboard={setData} sourceUrl={sourceUrl} setSourceUrl={setSourceUrl} rightsConfirmed={rightsConfirmed} setRightsConfirmed={setRightsConfirmed} addSource={addSource} removeSource={removeSource} saving={saving} notice={notice} />}
      </>}
    </section>
  </main>;
}
...
function SourcesPanel({ data, onDashboard, sourceUrl, setSourceUrl, rightsConfirmed, setRightsConfirmed, addSource, removeSource, saving, notice }: { data: DashboardData; onDashboard: (dashboard: DashboardData) => void; sourceUrl: string; setSourceUrl: (value: string) => void; rightsConfirmed: boolean; setRightsConfirmed: (value: boolean) => void; addSource: (event: React.FormEvent) => void; removeSource: (id: string) => void; saving: boolean; notice: string }) {
  const [librarySourceId, setLibrarySourceId] = useState(data.sourceChannels[0]?.id || '');
  ...
  async function loadPastVideos() {
    ...
    setLibraryLoading(true); setLibraryNotice(''); setSelectedIds([]);
    const response = await fetch(`/api/videos/backfill?sourceId=${encodeURIComponent(librarySourceId)}`);
    const body = await response.json(); setLibraryLoading(false);
    if (!response.ok) return setLibraryNotice(body.error || 'Past videos could not be loaded.');
    setPastVideos(body.videos || []);
    setLibraryNotice(body.videos?.length ? `Loaded ${body.videos.length} past videos. Choose only the ones you want.` : 'No public past videos were found for this source.');
  }
  ...
  async function queueSelected() {
    ...
    setLibraryLoading(true); setLibraryNotice('');
    const response = await fetch('/api/videos/backfill', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sourceId: librarySourceId, videoIds: selectedIds }) });
    const body = await response.json(); setLibraryLoading(false);
    if (!response.ok) return setLibraryNotice(body.error || 'Selected videos could not be queued.');
    if (body.dashboard) onDashboard(body.dashboard);
    const selected = new Set(selectedIds);
    setPastVideos((current) => current.map((video) => selected.has(video.id) ? { ...video, alreadyQueued: true } : video));
    setSelectedIds([]);
    setLibraryNotice(`${body.queued} video${body.queued === 1 ? '' : 's'} queued for analysis and publishing${body.skipped ? `; ${body.skipped} already existed` : ''}.`);
  }

  const selectableIds = pastVideos.filter((video) => !video.alreadyQueued).map((video) => video.id).slice(0, 20);
  return <section className="tab-panel sources-tab"><div className="tab-heading"><div><p className="overline">Always-on inputs</p><h2>Source channels</h2><p>ClipForge watches these channels, streams, and VODs for new moments.</p></div><span className="tab-count">{data.sourceChannels.length} / {data.tenant.sourceChannelLimit}</span></div><div className="sources-layout"><div className="source-roster">{data.sourceChannels.length ? data.sourceChannels.map((source) => <article key={source.id}><span><Radio /></span><div><b>{source.title}</b><p>{source.platform === 'twitch' ? 'Twitch' : 'YouTube'} · {source.handle || source.platformLogin || source.platformUserId}</p><small>Monitoring active</small></div><button aria-label={`Remove ${source.title}`} onClick={() => removeSource(source.id)} disabled={saving}><Trash2 /></button></article>) : <div className="empty-state"><Radio /><b>No sources connected</b><p>Add your first source to begin monitoring.</p></div>}</div><aside className="source-connect-card"><span className="feature-icon"><Plus /></span><h3>Add another source</h3><p>YouTube channels, uploads, live replays, Twitch channels, and VOD links work.</p><form className="channel-form" onSubmit={addSource}><label htmlFor="source-channel"><Link2 /> Source URL or handle</label><div><input id="source-channel" value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="youtube.com/@creator" required /><button aria-label="Connect source" disabled={saving || !rightsConfirmed || data.sourceChannels.length >= data.tenant.sourceChannelLimit}>{saving ? <LoaderCircle className="spin" /> : <ArrowRight />}</button></div><label className="rights-check"><input type="checkbox" checked={rightsConfirmed} onChange={(event) => setRightsConfirmed(event.target.checked)} /><ShieldCheck /> I own or have permission to repurpose this source.</label></form>{notice && <p className="form-notice">{notice}</p>}</aside></div><section className="past-library"><div className="past-library-head"><div><span className="feature-icon purple"><Tv /></span><div><p className="overline">Manual backfill</p><h3>Analyze and post from past videos</h3><p>Pick a source, choose specific uploads or VODs, and queue only those videos.</p></div></div><div className="past-library-controls"><select aria-label="Source for past videos" value={librarySourceId} onChange={(event) => { setLibrarySourceId(event.target.value); setPastVideos([]); setSelectedIds([]); setLibraryNotice(''); }}>{data.sourceChannels.map((source) => <option key={source.id} value={source.id}>{source.title} · {source.platform}</option>)}</select><button className="button button-dark" onClick={loadPastVideos} disabled={libraryLoading || !librarySourceId}>{libraryLoading ? <LoaderCircle className="spin" /> : <><Eye /> Browse past videos</>}</button></div></div>{libraryNotice && <p className="library-notice">{libraryNotice}</p>}{pastVideos.length > 0 && <><div className="library-select-row"><button onClick={() => setSelectedIds(selectedIds.length === selectableIds.length ? [] : selectableIds)}>{selectedIds.length === selectableIds.length && selectableIds.length ? 'Clear selection' : `Select first ${selectableIds.length}`}</button><span>{selectedIds.length} selected · maximum 20 per batch</span></div><div className="past-video-grid">{pastVideos.map((video) => <label className={`${video.alreadyQueued ? 'queued' : ''} ${selectedIds.includes(video.id) ? 'selected' : ''}`} key={video.id}><input type="checkbox" checked={selectedIds.includes(video.id)} onChange={() => toggleVideo(video.id)} disabled={video.alreadyQueued} />{video.thumbnailUrl ? <img src={video.thumbnailUrl} alt="" loading="lazy" /> : <span className="video-placeholder"><Play /></span>}<div><small>{video.alreadyQueued ? 'ALREADY QUEUED' : video.platform.toUpperCase()}</small><b>{video.title}</b><time>{video.publishedAt ? new Date(video.publishedAt).toLocaleDateString() : 'Past upload'}</time></div></label>)}</div><div className="queue-selected-bar"><div><b>{selectedIds.length} video{selectedIds.length === 1 ? '' : 's'} selected</b><span>ClipForge will analyze, caption, and publish using your current autopilot settings.</span></div><button className="button button-primary" onClick={queueSelected} disabled={libraryLoading || !selectedIds.length}>{libraryLoading ? <LoaderCircle className="spin" /> : <><WandSparkles /> Analyze &amp; post selected</>}</button></div></>}</section></section>;
}
